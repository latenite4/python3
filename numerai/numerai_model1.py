...
def main():
    ...
    subfilename = today.strftime("%m-%d-%Y")
    ...
    if MODEL_FILE.is_file():
        ...
        print(">Training model...@{:12.2f}s".format(time.time()))
        st =   time.time()
        model.fit(training_data[feature_names], training_data[TARGET_NAME],verbose=True)
        print(">Trained  model...@{:12.2f}s".format(time.time()-st))
    ...
    sfname="submission-"+subfilename
    tournament_data[PREDICTION_NAME].to_csv(sfname, header=True)

    print(f"submit this file: {sfname}")
    ring_bell()
